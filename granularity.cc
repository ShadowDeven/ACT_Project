...
int insert_state(struct State_Record& tmp, COVG_MAP_VEC& map_vec, Config_Map& config_map, vector<struct Test_Parems>& test_para_vec)
{
	...
	for (int i = 0; i < map_vec.size(); i++)
	{
		...
		struct State_Record tmp_cube(cwnd, ssthresh, srtt, rttvar, state, prev_state, 0);
...
int cal_range (struct RANGE_INFO & range_info, int granularity)
{
	...
	range_info.total = range_info.cwnd_range * range_info.ssth_range * range_info.rtt_range * range_info.rtvar_range * range_info.state_range * range_info.prev_state_range;
	return 0;
}
